...
class ImportTypeBerlinBrandenburg implements ImportType
{
    private const FILE_PATTERN = '/^Antworten_für_Freitextaufgaben_\d{5}_(\d{4})_(\d{5})_[[:alpha:]]{1,2}_\d_Versuch_\d\.pdf$/u';
    ...
    public function assignFiles(array $files): ImportResult
    {
        foreach ($files as $file) {
            $login = preg_match(self::FILE_PATTERN, $file->getFileName(), $matches) ? $matches[2] . $matches[1] : null;
            if ($login) {
    ...
    public function columns(): array
    {
        return [
            ...
            'comment' => new Column('text', $this->lng->txt('essay_import_column_comments')),
        ];
    }
